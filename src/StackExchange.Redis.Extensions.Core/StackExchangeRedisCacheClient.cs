...
namespace StackExchange.Redis.Extensions.Core
{
    ...
    public class StackExchangeRedisCacheClient : ICacheClient
    {
        ...
        public StackExchangeRedisCacheClient(ISerializer serializer, ConfigurationOptions configuration = null)
        {
            if (serializer == null)
            {
            ...
            configuration = configuration ?? new ConfigurationOptions
            {
                Ssl = false,
                AllowAdmin = true,
            ...
            Database = connectionMultiplexer.GetDatabase(configuration.DefaultDatabase ?? 0);
        ...
        /// <param name="serializer">The serializer.</param>
        /// <param name="configuration">The configuration.</param>
        public StackExchangeRedisCacheClient(ISerializer serializer, IOptions<ConfigurationOptions> configurationOptions = null)
        {
            if (serializer == null)
            {
                throw new ArgumentNullException(nameof(serializer));
            ...
            var configuration = configurationOptions?.Value ?? new ConfigurationOptions
            {
                Ssl = false,
                AllowAdmin = true,
                Password = string.Empty,
