...
namespace StackExchange.Redis.Extensions.Core.Implementations
{
    ...
    public partial class RedisDatabase : IRedisDatabase
    {
        ...
        /// <inheritdoc/>
        public async Task<object> GetAsync(string key, Type returnType, DateTimeOffset expiresAt, CommandFlags flag = CommandFlags.None)
        {
            var valueBytes = await Database.StringGetAsync(key, flag).ConfigureAwait(false);
        ...
        /// <inheritdoc/>
        public async Task<object> GetAsync(string key, Type returnType, TimeSpan expiresIn, CommandFlags flag = CommandFlags.None)
        {
            var valueBytes = await Database.StringGetAsync(key, flag).ConfigureAwait(false);
