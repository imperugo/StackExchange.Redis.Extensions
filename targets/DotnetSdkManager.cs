...
namespace Targets
{
    internal class DotnetSdkManager
    {
        ...
        private static (bool customSdk, string sdkPath, string sdkVersion) EnsureRequiredSdkIsInstalled()
        {
            ...
            using var response = client.GetAsync(installScriptUrl).GetAwaiter().GetResult();

            var pathToSave = Path.Combine(BUILD_SUPPORT_DIR, installScriptName);
